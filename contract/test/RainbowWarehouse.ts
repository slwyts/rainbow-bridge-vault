...
const parseUsdt = (amount: string | number) =>
  parseUnits(String(amount), USDT_DECIMALS);
const parseXwaifu = (amount: string | number) =>
  parseUnits(String(amount), XWAIFU_DECIMALS);
...
describe("RainbowWarehouse", async () => {
  ...
  beforeEach(async () => {
    ...
    usdt = await viem.deployContract("MockERC20", [
      "USDT",
      "USDT",
      USDT_DECIMALS,
    ]);
    ...
    xwaifu = await viem.deployContract("MockERC20", [
      "xwaifu",
      "XWAIFU",
      XWAIFU_DECIMALS,
    ]);
  ...
  describe("U本位周期派发 - createDeposit", () => {
    beforeEach(async () => {
      const user1Usdt = await viem.getContractAt("MockERC20", usdtAddress, {
        client: { wallet: user1 },
      });
    ...
    it("应该成功创建存款 (10期, 0.5%费用)", async () => {
      const user1Warehouse = await viem.getContractAt(
        "RainbowWarehouse",
        warehouseAddress,
        {
          client: { wallet: user1 },
    ...
    it("应该拒绝期数为0的存款", async () => {
      ...
      await assert.rejects(
        user1Warehouse.write.createDeposit([
          usdtAddress,
          parseUsdt("10"),
          86400n,
          0,
  ...
  describe("U本位周期派发 - withdraw", () => {
    ...
    it("应该拒绝非所有者提款", async () => {
      ...
      const user2Warehouse = await viem.getContractAt(
        "RainbowWarehouse",
        warehouseAddress,
        {
          client: { wallet: user2 },
  ...
  describe("币本位锁仓 - ERC20", () => {
    beforeEach(async () => {
      const user1Xwaifu = await viem.getContractAt("MockERC20", xwaifuAddress, {
        client: { wallet: user1 },
      });
    ...
    it("应该成功创建 ERC20 锁仓", async () => {
      ...
      await user1Warehouse.write.createLockup([
        xwaifuAddress,
        amount,
        unlockTime,
      ]);
  ...
  describe("币本位锁仓 - 原生代币", () => {
    it("应该成功创建原生代币锁仓", async () => {
      ...
      await user1Warehouse.write.createLockup([zeroAddress, 0n, unlockTime], {
        value: amount,
      });
    ...
    it("应该成功提取原生代币锁仓", async () => {
      ...
      assert.strictEqual(
        balanceAfter - balanceBefore + gasUsed,
        expectedLocked
      );
    });
  });
